...
object Practice {
  ...
  def firstN(xs: List[Int], n: Int): List[Int] = {
    ...
    else xs.head :: firstN(xs.tail, n - 1)
  }
  ...
  def maxValue(xs: List[Int]): Int = xs match {
    ...
    case i :: tail =>
      val rest = maxValue(tail)
      if (i > rest) i else rest
  }
  ...
  def myFilter[A](xs: List[A], f: A => Boolean): List[A] = {
    def normalFilter(xs: List[A]): List[A] = {
      xs match {
        case Nil => Nil
        case i :: tail =>
          if (f(i)) i :: normalFilter(tail)
          else normalFilter(tail)
      }
    }

    val filtered = normalFilter(xs)

    def keepEven(xs: List[A]): List[A] = {
      xs match {
        case Nil => Nil
        case i1 :: _ :: tail => i1 :: keepEven(tail)
        case i :: _ => List(i)
      }
    }
